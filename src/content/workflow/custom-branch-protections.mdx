---
title: Advanced Branch Protection with Mergify Workflow Automation
...
3. **Streamlined Workflows**: Combine branch protection with other workflow
...
- Write custom checks that are posted by Mergify on pull requests like a CI
  would do. This custom checks can also be used as conditions in the automatic
  merge conditions.
...
if the PR has a two approvals.

This more flexible than GitHub protected branch which only offers a fixed
number of approval for any pull request.
...
pull_request_rules:
  - name: Documentation PRs
    conditions:
      # At least one modified files in the docs/ directory
      - files ~= ^docs/
      # No other files are modifeid#
...
This allows pull request to be automatically merged with conditions that are
finer-grained that GitHub generic branch protections.

### Scheduling Merges

You can protect merges to happen only during office hours. For urgent fixes,
...
like a CI system would do, allowing to enforce extra rules on pull request
before merging.
    ...
    actions:
      post_check:
        success_condition:
          ...
          # (Note that if you're using this with GitHub branch protection,
...
- it has the label "hotfix", the author is part of the `devs` team and it has
  been approved by somebody else and it fixes an issue or revert a commit.

If one of those scenario is true, the check `Rule: PR Approval (post_check)`
